...
* **📈 Trends Dashboard:** Explore a dashboard of reviewer statistics, now rendered with lightweight ASCII charts for quick loading. A new "Trends" link is available in the top navigation. The figures come from a Ruby script that parses each `_reviewers` prompt and its JSON comment log, then writes aggregated numbers to `assets/data/trends.json` via GitHub Actions.
...
All statistics on the Trends page are derived from real comment data. Each prompt in `_reviewers/` has a matching JSON file that lists discussion comments from the original repository. The `scripts/generate_trends.rb` script parses these files, counts languages, labels, repositories, and timestamps, and writes a combined `trends.json`. A GitHub Actions workflow runs the script on every push so the data is always fresh.
