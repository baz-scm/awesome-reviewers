...
function buildSummaryStats(d){
  ...
  const topRepos  = Object.entries(d.repositories).sort((a,b)=>b[1]-a[1]).slice(0,8).length;
  const reuse8    = d.prompt_occ_bins['8+'] || 0;
  document.getElementById('stat-total-languages').textContent = langCount;
  document.getElementById('stat-category-count').textContent = catCount;
  document.getElementById('stat-top-repos').textContent = topRepos;
  document.getElementById('stat-reuse-8plus').textContent = reuse8;
}
